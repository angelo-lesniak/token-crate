...
several presets. Every preset states the GPU memory it needs in
...
declare 15 to 30 GiB; if none of them fits your card, [write one of your
...
| `qwen3.8-flash-next-q4` | Flash-Next Q4 | 128K | 1 | none | 30 GiB and 96 GiB RAM | 33 to 34 tokens/s | the [96 GB RAM tier](#the-96-gb-ram-tier) that the pinned build loads: 125B parameters, 6B active, experts in system memory |
...
llama.cpp build b11028, except `qwen3.8-flash-next-q4`, whose figure is
from [the 96 GB tier record](validation.md#the-settings-of-the-96-gb-ram-tier)
on build b10920, and the two 16 GB presets, which are from [their own
...
The default preset is `qwen3.8-27b-q4-mtp`, and one 64K slot serves a
single agent better than two 32K slots: with an extension set and `xhigh`
reasoning, a 32K slot is the smallest slot that holds the largest request
([reasoning effort](#reasoning-effort) says why the slot fills). Two
requests issued together take twice as long as one, at the same
generation speed, because the second waits for the slot; a coding session
of a dozen turns fills the 64K slot, and the agent then compacts its
...
until the running request has finished, then the router unloads the loaded
model and loads the other one, which reads the full file from disk again.
Keep one preset for a session. Switching between the two Flash-Next
presets replaces one 111 GB file in the page cache with the other.
...
retries that cost more in total. `gpt-oss-20b-fast` stays at `medium`
because speed is its purpose. These defaults apply to clients that send
...
prompt, so a 32K slot fills sooner; the single-slot presets, the default
among them, absorb this better than the two-slot `qwen3.8-27b-q4`.
...
- **In pi and oh-my-pi**, use the thinking level: `/thinking low` inside
  ...
  What pi sends at its seeded `xhigh` on gpt-oss is expected to be
  `high`; this is not validated. The GLM rows have not run.
...
The uncensored presets are listed like any other preset, in the built-in
chat UI, the agents' model lists, and here; their descriptions carry the
word "uncensored" and the method.
...
- `glm-5.3-flash-q2-uncensored` runs orcarouter's abliteration of
  ...
  decides whether it fits. It would swap through the memory map rather
  than crash.
...
| Generation | 33 to 34 tokens/s, measured | 6 to 7 tokens/s in one consumer report on an RTX 3090 |
| Prompt processing | 119 tokens/s short, 740 for a 6K or 12K prompt | 60 to 70 tokens/s in that same report |
| Status | loadable, validated | prepared, waits for a llama.cpp release with `glm5next`, unverified |

Flash-Next is the one to pick today. Its prompt processing is a quarter
of the 27B's, so a pi session's first request (about 8,000 tokens) works
out at about 11 seconds before the first word at the presets' 740 tokens
per second. That figure is arithmetic: the recorded agent session ran
before the micro-batch these presets set and took 43 seconds.
...
The Flash-Next presets use a 2048 micro-batch, which takes 2.2 GiB more
of the card than the default and more than doubles the long-prompt speed.
`load-mode = "none"` would double prompt processing again but reads the
experts into 58 GiB of memory the kernel cannot reclaim; the presets do
not set it, and the preset comment says how to try it. Neither uses MTP:
...
([Adding a model](#adding-a-model) describes the gate). The model is
shipped because it is expected to beat the 27B on knowledge, on the
...
Those are the full models. The 2-bit quant that fits keeps 78 percent
top-1 agreement with its own full model, so the edge over the 27B is
...
The two GLM presets carry the researched first attempt from
...
correct GLM output; the other presets are unaffected. The vision tower is
not used, because no projector is mounted.
...
KV cache, so f16 cost 1.6 GiB more and 7 percent of generation speed
...
1. Print a manifest draft for the GGUF file and save it under
   ...
   Choose a `<set>` name that no shipped manifest uses; `models list` shows
   the existing names.
   ...
   lists until the pin passes that build; `LLM_DEFAULT_PRESET` cannot
...
2. Copy the closest preset under `config/presets/`, point `model_set` at the
   ...
   Presets express llama-server settings through typed keys, the ones the
   shipped presets use; `tokencrate/presets.py` defines them, with each
   key's type and limits, and `presets render` reports the ones it does
   not know. An option the typed keys do not cover goes into
   the `[server.extra]` table as `name = value`, using the option's long
   name without the leading dashes (`verbose = true`); the renderer
   copies those lines unchanged into the preset's section of
   `build/models.ini`, after the typed keys. `presets render` names what
   it rejects; run it after every edit. It rejects:

   - an unknown key, and an extra option that spells a typed setting;
   ...
   - an extra option the renderer owns: the ones that tie a preset to the
     container and the router, remote RPC compute backends, and the ones
     that download model files at start, which [privacy and
     containment](privacy.md#what-leaves-the-machine)
     relies on.

   Review extra options as code. The table is an escape hatch for
   llama-server tuning, not a security boundary for untrusted presets;
   accepting an option does not establish that the pinned build supports it.
...
record](validation.md#the-16-gb-presets) but not yet on a 16 GB card. If
