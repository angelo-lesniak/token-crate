...
class SettingsTests(unittest.TestCase):
    ...
    def test_every_wrapper_default_reaches_subprocesses_with_its_effective_value(self) -> None:
        ...
        for key, default in env.parse_env_file(SOURCE_ROOT / ".env.example").items():
            if key not in env.STORAGE_KEYS and key != "LLM_PORT":
    ...
    def test_token_is_captured_and_never_reaches_a_subprocess_environment(self) -> None:
        ...
        for settings in (from_file, from_shell):
            self.assertNotIn("HF_TOKEN", settings.subprocess_env())
            self.assertNotIn("HF_TOKEN", settings.environ)
            self.assertNotIn("HF_TOKEN", settings.file_values)
        # An empty shell HF_TOKEN means "no token" and suppresses the file value.
    ...
    def test_env_keys_missing_from_the_example_file_are_reported(self) -> None:
        ...
        with contextlib.redirect_stderr(io.StringIO()) as stderr:
            env.load(self.tmp, {})
        self.assertEqual(
            stderr.getvalue(), "TokenCrate: .env: keys not listed in .env.example: LLM_ALLOW_REMOTE, LLM_BIND_ADDRESS\n"
        )
